...
# fi
scp -rq python/ pyproject.toml hronn@$target:$dir_application

# 2: Update dependencies, if requested
if [ "$install_dependencies" = true ] ; then
    printf "Updating dependencies...\n"
    ssh hronn@$target "cd $dir_application && poetry update"
fi

# 3: Run the application
printf "Running linak-mqtt application...\n--------------------------------------------------\n"
ssh hronn@$target "cd $dir_application && poetry run python -u python/__main_.py"